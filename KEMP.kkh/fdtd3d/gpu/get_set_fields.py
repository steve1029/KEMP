# Author  : Ki-Hwan Kim
# Purpose : Class to get/set the subdomains of fields
# Target  : GPU using PyOpenCL
# Created : 2011-11-02
# Modified: 2012-03-12 (add the GetFieldsSquareSum class)
...
class GetFields:
    ...
    def get_event(self, exec_kernel=True):
        if exec_kernel:
            self.enqueue_kernel()

        return cl.enqueue_copy(self.mainf.queue, self.host_array, self.target_buf, is_blocking=False)
...
class SetFields:
    ...
    def set_fields_spatial_value(self, value, wait_for=[]):
        ...
        cl.enqueue_copy(self.mainf.queue, \
                        self.source_buf, value, \
    ...
    def set_fields_single_value(self, value, wait_for=[]):
        ...
        self.program.copy( \
            self.mainf.queue, (self.gs,), (self.mainf.ls,), \
            nx, ny, nz, np.int32(0), \
            self.target_bufs[0], self.mainf.dtype(value), \
            wait_for=wait_for)
        ...
        for target_buf in self.target_bufs[1:]:
            self.program.copy( \
                self.mainf.queue, (self.gs,), (self.mainf.ls,), \
                nx, ny, nz, np.int32(0), \
                target_buf, self.mainf.dtype(value))
