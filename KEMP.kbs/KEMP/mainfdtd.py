...
#            set_coordinate(self, x_offset)
    ...
    def set_structures_single(self, structures=None, wait=True):
        ...
        if structures is not None:
            ...
            code_prev = structures[-1].code_prev
            code_post = structures[-1].code_list[self.materials_classification]
            template = self.engine.templates['structures']
            # OpenMP settings
            if 'cpu' in self.engine.name:
                code_prev += ['__COEFFICIENTS__']
                code_post += ['ce1s, ce2s']
            code     = template_to_code(template, \
                                        code_prev+self.engine.code_prev, \
                                        code_post+self.engine.code_post)
            self._structure_prg = self.engine.build(code)
            for ist, structure in enumerate(structures):
                print 'setting structures %d / %d' % (ist+1, len(structures))
...
#                        if pt0[i] == 0: pt0[i] = 1
                    if region_true:
                        ...
                        if 'opencl' in self.engine.name:
                            evt = self._structure_prg.mark_to_mk_2d(*args)
                        elif 'cuda' in self.engine.name:
                            func = self.engine.get_function(self._structure_prg, 'mark_to_mk_2d')
                            self.engine.prepare(func, args)
                            evt = self.engine.enqueue_kernel(func, args)
                        elif  'cpu' in self.engine.name:
                            self.engine.set_kernel(self._structure_prg.mark_to_mk_2d, args)
                            evt = self._structure_prg.mark_to_mk_2d(*(args[3:]))
                        evt.wait()
                elif '3D' in self.mode:
...
#                        if pt0[i] == 0: pt0[i] = 1
                    if region_true:
                        ...
                        args = [self.engine.queue, (cpwd.gs,), (self.engine.ls,), \
                                ...
                                cpwd.mx.data, cpwd.my.data, cpwd.mz.data]
                        if 'opencl' in self.engine.name:
                            evt = self._structure_prg.mark_to_mk_3d(*args)
                        elif 'cuda' in self.engine.name:
                            func = self.engine.get_function(self._structure_prg, 'mark_to_mk_3d')
                            self.engine.prepare(func, args)
                            evt = self.engine.enqueue_kernel(func, args)
                        elif  'cpu' in self.engine.name:
                            self.engine.set_kernel(self._structure_prg.mark_to_mk_3d, args)
                            self._structure_prg.mark_to_mk_3d(*(args[3:]))
                            evt = FakeEvent()
                        evt.wait()
                self.mkex.release_data()
                self.mkey.release_data()
                self.mkez.release_data()
                fd_buf = np.zeros(self.shape, dtype=self.dtype)
                for fd in self.ehs:
                    fd.set_buffer_from_FDTD(fd_buf, self)
...
class FDTD_multi_devices:
        ...
        if pbc_apply['x']:
            ...
            for sfdtd in self.fdtd_group:
                if 'opencl' in sfdtd.engine.name:
                    ...
                    gs    = self.ny*self.nz
                for part in self.complex_parts:
                    sfdtd.engine.kernel_args['buf_to_field_bbc']['ey'][part] = [sfdtd.engine.queue, (gs,), (sfdtd.engine.ls,), \
                                                                                np.int32(0), np.int32(0*2+0), np.int32(1*2+0), \
                                                                                np.int32(sfdtd.nx), np.int32(sfdtd.ny), np.int32(sfdtd.nz), \
                                                                                comp_to_real(self.dtype)(-self.sin_klx), \
                                                                                comp_to_real(self.dtype)(+self.cos_klx), \
                                                                                sfdtd.ey.__dict__[part].data, sfdtd.buff_recv_e]
                    sfdtd.engine.kernel_args['buf_to_field_bbc']['ez'][part] = [sfdtd.engine.queue, (gs,), (sfdtd.engine.ls,), \
                                                                                np.int32(0), np.int32(0*2+1), np.int32(1*2+1), \
                                                                                np.int32(sfdtd.nx), np.int32(sfdtd.ny), np.int32(sfdtd.nz), \
                                                                                comp_to_real(self.dtype)(-self.sin_klx), \
                                                                                comp_to_real(self.dtype)(+self.cos_klx), \
                                                                                sfdtd.ez.__dict__[part].data, sfdtd.buff_recv_e]
                    sfdtd.engine.kernel_args['buf_to_field_bbc']['hy'][part] = [sfdtd.engine.queue, (gs,), (sfdtd.engine.ls,), \
                                                                                np.int32(1), np.int32(0*2+0), np.int32(1*2+0), \
                                                                                np.int32(sfdtd.nx), np.int32(sfdtd.ny), np.int32(sfdtd.nz), \
                                                                                comp_to_real(self.dtype)(+self.sin_klx), \
                                                                                comp_to_real(self.dtype)(+self.cos_klx), \
                                                                                sfdtd.hy.__dict__[part].data, sfdtd.buff_recv_h]
                    sfdtd.engine.kernel_args['buf_to_field_bbc']['hz'][part] = [sfdtd.engine.queue, (gs,), (sfdtd.engine.ls,), \
                ...
                if 'opencl' in sfdtd.engine.name:
                    sfdtd.engine.kernels['buf_to_field_bbc'] = sfdtd.engine.programs['get_set_data'].buf_to_field_bbc
                elif 'cuda' in sfdtd.engine.name:
                    sfdtd.engine.kernels['buf_to_field_bbc'] = sfdtd.engine.get_function(sfdtd.engine.programs['get_set_data'], 'buf_to_field_bbc')
    ...
    def apply_TFSF_multi(self, region, boundary={'x':'+-', 'y':'+-', 'z':'+-'}, material=None, is_oblique=False):
        from incident import TFSF_Boundary
        tfsf = TFSF_Boundary(self, region, boundary, is_oblique)
        self.tfsfs.append(tfsf)
        self.is_tfsf = True
    ...
    def set_structures_multi(self, structures=None):
        ...
        for structure in structures:
            if structure.material.classification in ['dielectric', 'electric dispersive', 'electromagnetic dispersive']:
                self.is_electric = True
            if structure.material.classification in ['dimagnetic', 'magnetic dispersive', 'electromagnetic dispersive']:
                self.is_magnetic = True
            if structure.material.classification in ['electric dispersive', 'magnetic dispersive', 'electromagnetic dispersive']:
                self.is_dispersive = True
        real_dtype = comp_to_real(self.dtype)
    ...
    def send_e_multi(self, wait=True):
        ...
        for i in xrange(self.num_devices):
            ...
            if i == self.num_devices-1:
                sfdtd_recv = self.fdtd_group[0  ]
            else:
                sfdtd_recv = self.fdtd_group[i+1]
            nbytes_buf = sfdtd.temp_send_e.nbytes
...
#                    evt = sfdtd.engine.enqueue(sfdtd.engine.drv.memcpy_dtoh_async, [sfdtd.temp_send_e, sfdtd.buff_send_e], sfdtd.engine.stream)
                    evt = sfdtd.engine.enqueue(sfdtd.engine.drv.memcpy_peer_async, [sfdtd_recv.buff_recv_e, sfdtd.buff_send_e, nbytes_buf, sfdtd_recv.engine.ctx, sfdtd.engine.ctx, sfdtd.engine.stream])
                    evts.append(evt)
        if wait: wait_for_events(self, evts)
        return evts
    ...
    def comm_e_multi(self):
        pass
        '''
        if self.pbc_x and 'opencl' in self.engine_name:
            if not self.is_complex:
...
#                        evts.append(evt)
    ...
    def send_h_multi(self, wait=True):
        ...
        for i in xrange(self.num_devices):
            sfdtd = self.fdtd_group[i]
            if i == 0:
                sfdtd_recv = self.fdtd_group[ -1]
            else:
                sfdtd_recv = self.fdtd_group[i-1]
            nbytes_buf = sfdtd.temp_send_h.nbytes
            for j, part in enumerate(self.complex_parts):
                if 'opencl' in sfdtd.engine.name:
...
#                    evt = sfdtd.engine.enqueue(sfdtd.engine.drv.memcpy_dtoh_async, [sfdtd.temp_send_h, sfdtd.buff_send_h], sfdtd.engine.stream)
                    evt = sfdtd.engine.enqueue(sfdtd.engine.drv.memcpy_peer_async, [sfdtd_recv.buff_recv_h, sfdtd.buff_send_h, nbytes_buf, sfdtd_recv.engine.ctx, sfdtd.engine.ctx, sfdtd.engine.stream])
                    evts.append(evt)
        wait_for_events(self, evts)
...
class FDTD_MPI:
    ...
    def set_structures_mpi(self, structures=None):
        if self.worker and structures is not None:
            if   self.sub_fdtd.extension == 'single':
                self.sub_fdtd.set_structures_single(structures)
            elif self.sub_fdtd.extension == 'multi':
...
#        wait_for_events(self, evts)
    ...
    def updateE_mpi_overlap(self, wait=True):
        ...
        if not (self.is_pml and '+' in self.pml_apply['x'] and self.last):
        ...
        self.updateE_exc0_time = t1 - t0
        self.updateE_exc1_time = t6 - t5
    ...
    def updateH_mpi_nonblock(self, wait=True):
        ...
        self.updateH_calc_time = t1 - t0
        self.updateH_buff_time = 0.
        self.updateH_exc0_time = t2 - t1
        self.updateH_exc1_time = t4 - t3
        self.updateH_comm_time = t3 - t2
        self.updateH_time      = t4 - t0
    ...
    def updateH_mpi_overlap(self, wait=True):
        ...
        t2 = dtm.now()
        wait_for_events(self, evts_get)
        if not (self.is_pml and '-' in self.pml_apply['x'] and self.first):
            self.get_fields_to_buff_fdtd('-')
        ...
        self.updateH_exc0_time = t1 - t0
        self.updateH_exc1_time = t6 - t5
...
class Basic_FDTD_3d(FDTD_single_device, FDTD_multi_devices, FDTD_MPI):
    def __init__(self, mode, space_grid, dtype=np.float32, engine='nvidia_opencl', device_id=0, MPI_extension=False, is_subfdtd=False):
        ...
        self.is_pml     = False
        self.is_pbc     = False
        self.pml_apply = {'x':'', 'y':'', 'z':''}
        self.pbc_apply = {'x':False, 'y':False, 'z':False}
        if len(space_grid) != 3:
            raise IndexError, '2d-fdtd space must have 2d-space grids. Please check the input variable "space_grid"'
...
def FDTDspace(mode, space_grid, dtype=np.float32, engine='nvidia_cuda', device_id=0, MPI_extension=False):
    return Basic_FDTD(mode, space_grid, dtype, engine, device_id, MPI_extension)
