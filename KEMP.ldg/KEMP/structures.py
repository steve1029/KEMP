...
class Box(Structure):
    def __init__(self, material, coord, rot_axis=(0.,0.,1.), angle=0., origin=None):
        Structure.__init__(self)
        point_ld, point_ru = coord # left_down, right_up
        self.point_ld = np.array(point_ld)
        self.point_ru = np.array(point_ru)
        ...
        self.rot_mat   = rotation_matrix(rot_axis, angle)
        if origin==None: self.origin = ((self.points[0]+self.points[3])/2.,(self.points[1]+self.points[4])/2.,(self.points[2]+self.points[5])/2. )
        else           : self.origin = origin
        self.material = material
