...
class Structure:
    ...
    def set_fdtd(self, fdtd):
        ...
        self.prg = fdtd.engine.build(code)
    ...
    def grids_list(self, fdtd):
        ...
        return grids_lists
'''
class multiple_structure(structure):
    def __init__(self):
        pass
    ...
    def set_dimagnetic(self):
        pass

    def set_electric_dispersive(self):
        pass

    def set_magnetic_dispersive(self):
        pass

    def set_electromagnetic_dispersive(self):
        pass
'''
# 2D structures
    ...
    def set_structure(self, fdtd, wait=True):
        ...
        for i in xrange(3):
            ...
            if 'opencl' in fdtd.engine.name:
                evt  = self.prg.set_struc_poly(*args)
            elif 'cuda' in fdtd.engine.name:
                func = fdtd.engine.get_function(self.prg, 'set_struc_poly')
                fdtd.engine.prepare(func, args)
                evt  = fdtd.engine.enqueue_kernel(func, args, False)
            elif  'cpu' in fdtd.engine.name:
                func = fdtd.engine.set_kernel(self.prg.set_struc_poly, args)
                evt  = func(*(args[3:]))
            evts.append(evt)
    ...
    def set_structure(self, fdtd, wait=True):
        ...
        for i in xrange(3):
            ...
            args = [fdtd.engine.queue, (fdtd.engine.gs,), (fdtd.engine.ls,), \
                    ...
                    material_params.data] + self.coefs_lists[i]
            if 'opencl' in fdtd.engine.name:
                evt  = self.prg.set_struc_rect(*args)
            elif 'cuda' in fdtd.engine.name:
                func = fdtd.engine.get_function(self.prg, 'set_struc_rect')
                fdtd.engine.prepare(func, args)
                evt  = fdtd.engine.enqueue_kernel(func, args, False)
            elif  'cpu' in fdtd.engine.name:
                func = fdtd.engine.set_kernel(self.prg.set_struc_rect, args)
                evt  = func(*(args[3:]))
            evts.append(evt)
        if wait: wait_for_events(fdtd, evts)
...
# 3D structures
class Quadrangular_prism(Structure):
    def __init__(self, material, base_com, length_width, height, axis, rot_axis=(0.,0.,1.), angle=0., origin=None):
        Structure.__init__(self)
        self.name = 'quadrangular_prism'
        self.base_com = np.array(base_com, dtype=np.float64)
        self.radius   = np.array(length_width, dtype=np.float64)
    ...
    def set_structure(self, fdtd, wait=True):
        ...
        length_width    = Fields(fdtd, (2,), dtype=comp_to_real(fdtd.dtype), mem_flag='r', init_value=self.radius/fdtd.min_ds)
        ...
        for i in xrange(3):
            material_params = Fields(fdtd, (len(self.material.params[i]),), dtype=comp_to_real(fdtd.dtype), mem_flag='r', \
                                     init_value=np.array(self.material.params[i], dtype=comp_to_real(fdtd.dtype)))
            args = [fdtd.engine.queue, (fdtd.engine.gs,), (fdtd.engine.ls,), \
                    com.data, length_width.data, height, np.int32(self.axis), \
                    ...
                    material_params.data] + self.coefs_lists[i]
            if 'opencl' in fdtd.engine.name:
                evt  = self.prg.set_struc_recd(*args)
            elif 'cuda' in fdtd.engine.name:
                func = fdtd.engine.get_function(self.prg, 'set_struc_recd')
                fdtd.engine.prepare(func, args)
                evt  = fdtd.engine.enqueue_kernel(func, args, False)
            elif  'cpu' in fdtd.engine.name:
                func = fdtd.engine.set_kernel(self.prg.set_struc_recd, args)
                evt  = func(*(args[3:]))
            evts.append(evt)
        if wait: wait_for_events(fdtd, evts)
        del com, length_width, origin, rot_mat, material_params
        return evts
    ...
    def set_structure(self, fdtd, wait=True):
        ...
        for i in xrange(3):
            ...
            args = [fdtd.engine.queue, (fdtd.engine.gs,), (fdtd.engine.ls,), \
                    ...
                    material_params.data] + self.coefs_lists[i]
            if 'opencl' in fdtd.engine.name:
                evt = self.prg.set_struc_ellp3d(*args)
            elif 'cuda' in fdtd.engine.name:
                func = fdtd.engine.get_function(self.prg, 'set_struc_ellp3d')
                fdtd.engine.prepare(func, args)
                evt  = fdtd.engine.enqueue_kernel(func, args, False)
            elif  'cpu' in fdtd.engine.name:
                func = fdtd.engine.set_kernel(self.prg.set_struc_ellp3d, args)
                evt  = func(*(args[3:]))
            evts.append(evt)
        if wait: wait_for_events(fdtd, evts)
    ...
    def set_structure(self, fdtd, wait=True):
        ...
        for i in xrange(3):
            ...
            args = [fdtd.engine.queue, (fdtd.engine.gs,), (fdtd.engine.ls,), \
                    ...
                    material_params.data] + self.coefs_lists[i]
            if 'opencl' in fdtd.engine.name:
                evt  = self.prg.set_struc_elcd(*args)
            elif 'cuda' in fdtd.engine.name:
                func = fdtd.engine.get_function(self.prg, 'set_struc_elcd')
                fdtd.engine.prepare(func, args)
                evt  = fdtd.engine.enqueue_kernel(func, args, False)
            elif  'cpu' in fdtd.engine.name:
                func = fdtd.engine.set_kernel(self.prg.set_struc_elcd, args)
                evt  = func(*(args[3:]))
            evts.append(evt)
        if wait: wait_for_events(fdtd, evts)
        del com, radius, origin, rot_mat, material_params
...
class Polyprism(Structure):
    ...
    def set_structure(self, fdtd, wait=True):
        self.set_fdtd(fdtd)
        height    = fdtd.dtype(self.height/fdtd.min_ds)
        ...
        for i in xrange(3):
            ...
            args = [fdtd.engine.queue, (fdtd.engine.gs,), (fdtd.engine.ls,), \
                    ...
                    material_params.data] + self.coefs_lists[i]
            if 'opencl' in fdtd.engine.name:
                evt  = self.prg.set_struc_plpm(*args)
            elif 'cuda' in fdtd.engine.name:
                func = fdtd.engine.get_function(self.prg, 'set_struc_plpm')
                fdtd.engine.prepare(func, args)
                evt  = fdtd.engine.enqueue_kernel(func, args, False)
            elif  'cpu' in fdtd.engine.name:
                func = fdtd.engine.set_kernel(self.prg.set_struc_plpm, args)
                evt  = func(*(args[3:]))
            evts.append(evt)
        if wait: wait_for_events(fdtd, evts)
    ...
    def set_structure(self, fdtd, wait=True):
        ...
        for i in xrange(3):
            ...
            args = [fdtd.engine.queue, (fdtd.engine.gs,), (fdtd.engine.ls,), \
                    ...
                    material_params.data] + self.coefs_lists[i]
            if 'opencl' in fdtd.engine.name:
                evt  = self.prg.set_struc_boxs(*args)
            elif 'cuda' in fdtd.engine.name:
                func = fdtd.engine.get_function(self.prg, 'set_struc_boxs')
                fdtd.engine.prepare(func, args)
                evt  = fdtd.engine.enqueue_kernel(func, args, False)
            elif  'cpu' in fdtd.engine.name:
                func = fdtd.engine.set_kernel(self.prg.set_struc_boxs, args)
                evt  = func(*(args[3:]))
            evts.append(evt)
        if wait: wait_for_events(fdtd, evts)
        del points, material_params
    ...
    def set_structure(self, fdtd, wait=True):
        ...
        for i in xrange(3):
            ...
            args = [fdtd.engine.queue, (fdtd.engine.gs,), (fdtd.engine.ls,), \
                    ...
                    material_params.data] + self.coefs_lists[i]
            if 'opencl' in fdtd.engine.name:
                evt  = self.prg.set_struc_elcn(*args)
            elif 'cuda' in fdtd.engine.name:
                func = fdtd.engine.get_function(self.prg, 'set_struc_elcn')
                fdtd.engine.prepare(func, args)
                evt  = fdtd.engine.enqueue_kernel(func, args, False)
            elif  'cpu' in fdtd.engine.name:
                func = fdtd.engine.set_kernel(self.prg.set_struc_elcn, args)
                evt  = func(*(args[3:]))
            evts.append(evt)
        if wait: wait_for_events(fdtd, evts)
        del com, radius, origin, rot_mat, material_params
...
class Elliptic_truncated_cone(Structure):
    ...
    def set_structure(self, fdtd, wait=True):
        self.set_fdtd(fdtd)
        height   = fdtd.dtype(self.height  /fdtd.min_ds)
        t_height = fdtd.dtype(self.t_height/fdtd.min_ds)
        com     = Fields(fdtd, (3,), dtype=comp_to_real(fdtd.dtype), mem_flag='r', init_value=self.com   /fdtd.min_ds)
        radius  = Fields(fdtd, (2,), dtype=comp_to_real(fdtd.dtype), mem_flag='r', init_value=self.radius/fdtd.min_ds)
        origin  = Fields(fdtd, (3,), dtype=comp_to_real(fdtd.dtype), mem_flag='r', init_value=self.origin/fdtd.min_ds)
        rot_mat = Fields(fdtd, (9,), dtype=comp_to_real(fdtd.dtype), mem_flag='r', init_value=self.rot_mat           )
        ...
        for i in xrange(3):
            ...
            args = [fdtd.engine.queue, (fdtd.engine.gs,), (fdtd.engine.ls,), \
                    ...
                    material_params.data] + self.coefs_lists[i]
            if 'opencl' in fdtd.engine.name:
                evt = self.prg.set_struc_eltrcn(*args)
            elif 'cuda' in fdtd.engine.name:
                func = fdtd.engine.get_function(self.prg, 'set_struc_eltrcn')
                fdtd.engine.prepare(func, args)
                evt  = fdtd.engine.enqueue_kernel(func, args, False)
            elif  'cpu' in fdtd.engine.name:
                func = fdtd.engine.set_kernel(self.prg.set_struc_eltrcn, args)
                evt  = func(*(args[3:]))
            evts.append(evt)
        if wait: wait_for_events(fdtd, evts)
        del com, radius, origin, rot_mat, material_params
    ...
    def set_structure(self, fdtd, wait=True):
        ...
        for i in xrange(3):
            ...
            args = [fdtd.engine.queue, (fdtd.engine.gs,), (fdtd.engine.ls,), \
                    ...
                    material_params.data] + self.coefs_lists[i]
            if 'opencl' in fdtd.engine.name:
                evt  = self.prg.set_struc_plpy(*args)
            elif 'cuda' in fdtd.engine.name:
                func = fdtd.engine.get_function(self.prg, 'set_struc_plpy')
                fdtd.engine.prepare(func, args)
                evt  = fdtd.engine.enqueue_kernel(func, args, False)
            elif  'cpu' in fdtd.engine.name:
                func = fdtd.engine.set_kernel(self.prg.set_struc_plpy, args)
                evt  = func(*(args[3:]))
            evts.append(evt)
        if wait: wait_for_events(fdtd, evts)
